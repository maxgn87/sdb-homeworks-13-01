...
1. SYN - nmap -sS 192.168.133.130
   По протоколу TCP отправляется пакет с флагом ACK. Если что-то прослушивает порт, возвращается пакет с флагом SYN.
   Тогда для завершения отправляется пакет с RST.
3. FIN - nmap -sA 192.168.133.130
   По протоколу TCP отправляет пакет с флагом FIN. Если нет ответа, то отображает информацию по порту.
